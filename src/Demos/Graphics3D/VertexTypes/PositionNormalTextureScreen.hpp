...
// Demonstrates VertexPositionNormalTexture (Position + Normal + UV) -- the
// vertex type BasicEffect actually needs for a genuinely LIT + textured
// surface (confirmed via cna/examples/easygl_basiceffect_combinations_test
// .cpp: VertexPositionColor/VertexPositionTexture have no lit shader
// combination on this backend; only the normal-carrying vertex types do).
// Space/Enter/A toggles LightingEnabled live so the difference is provable,
// not just asserted.
class PositionNormalTextureScreen : public DemoScreen {
public:
    ...
    void LoadContent() override {
        ...
        effect_->DirectionalLight0.setDirectionProperty(Vector3(-0.5f, -0.6f, -0.6f));
...
protected:
    ...
    void OnDemoInput(InputState& input) override {
        ...
        if (input.IsMenuSelect(ControllingPlayer(), playerIndex)) {
            lit_ = !lit_;
            effect_->setLightingEnabledProperty(lit_);
        }
    }

    void OnDemoDraw(const GameTime&, SpriteBatch& sb, SpriteFont& font) override {
        std::vector<std::string> lines;
        lines.push_back("VertexPositionNormalTexture: Position + Normal + UV -- the vertex type");
        lines.push_back("BasicEffect actually needs for a lit surface (Color/Texture-only have no");
        lines.push_back("lit shader path). Space/Enter/A: LightingEnabled=" +
                         std::string(lit_ ? "true" : "false"));
